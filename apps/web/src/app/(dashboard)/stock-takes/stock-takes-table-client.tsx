...
} from "@/features/stock-takes/api";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Search, Plus, ChevronLeft, ChevronRight } from "lucide-react";

export function StockTakesTableClient({
  initialData,
}: {
  initialData: { items: StockTake[]; total: number; page: number; pageSize: number; hasMore: boolean };
}) {
  ...
  const query = useStockTakes(params);
  const data = (page === 1 && !search && !status && !dateFrom && !dateTo && initialData)
    ? initialData
    : query.data;
  const isLoading = data === initialData ? false : query.isLoading;
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input placeholder="Tìm theo số phiếu, ghi chú..." className="pl-10" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
        </div>
